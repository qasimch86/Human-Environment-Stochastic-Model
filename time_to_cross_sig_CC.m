...
d_sig=0.02;sigmax=0.7;lsig=0:d_sig:sigmax;
% ksig=0.1;
Nimx=100;
% T_cross1=zeros(length(lsig),Nimx);T_cross2=zeros(length(lsig),Nimx);
% T_cross3=zeros(length(lsig),Nimx);T_cross4=zeros(length(lsig),Nimx);
for ksig=1:length(lsig)
for Ni=1:Nimx
n=0.1;% Social norms
% n(2)=ln(kn);
% n(3)=ln(kn);
...
tcross=zeros(1,Np);T_cross_avg=zeros(Np,length(lsig));
for i=1:Np
    if i==1
        S(1,i)=K(i)-15;
    else
...
for i=1:Np
    T_cross(i,ksig,Ni)=tcross(i);
end
% T_cross1(ksig,Ni)=tcross(1);T_cross2(ksig,Ni)=tcross(2);
% T_cross3(ksig,Ni)=tcross(3);T_cross4(ksig,Ni)=tcross(4);

%     if sum(num_SIT(:,k))==0
%         T(k,ksig,ksig)=t+1;
%         T_cross(k,ksig,ksig)=t+1;
%     end
end
ksig
end
for ksig=1:length(lsig)
    for i=1:Np
        if isempty(nonzeros(T_cross(i,ksig,:)))
            T_cross_avg(i,ksig)=Nt;
        else
            T_cross_avg(i,ksig)=sum(T_cross(i,ksig,:))/length(nonzeros(T_cross(i,ksig,:)));% average
        end
    end
end
% for ksig=1:length(lsig)
%     if isempty(nonzeros(T_cross1(ksig,:)))
%         T_cross1_avg(ksig)=Nt;
%     else
%         T_cross1_avg(ksig)=sum(T_cross1(ksig,:))/length(nonzeros(T_cross1(ksig,:)));% average
%     end
%     if isempty(nonzeros(T_cross2(ksig,:)))
%         T_cross2_avg(ksig)=Nt;
%     else
%         T_cross2_avg(ksig)=sum(T_cross2(ksig,:))/length(nonzeros(T_cross2(ksig,:)));% average
%     end
%     if isempty(nonzeros(T_cross3(ksig,:)))
%         T_cross3_avg(ksig)=Nt;
%     else
%         T_cross3_avg(ksig)=sum(T_cross3(ksig,:))/length(nonzeros(T_cross3(ksig,:)));% average
%     end
%     if isempty(nonzeros(T_cross4(ksig,:)))
%         T_cross4_avg(ksig)=Nt;
%     else
%         T_cross4_avg(ksig)=sum(T_cross4(ksig,:))/length(nonzeros(T_cross4(ksig,:)));% average
%     end
% end
% figure
% hold on
% for i=1:Np
%     plot(lsig,T_cross_avg(i,:)/52,'Color',[1/i i/Np 1/(Np-i+1)]);
% end
CODE_Figure(lsig,T_cross_avg/52,'Time to Crosspatch');%,S2,S3,S4);
toc
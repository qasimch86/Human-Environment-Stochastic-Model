...
d_f=0.05;nmax=1;lf=0:d_f:nmax;
Nimx=100;
% T_cross1=zeros(length(lf),Nimx);T_cross2=zeros(length(lf),Nimx);
% T_cross3=zeros(length(lf),Nimx);T_cross4=zeros(length(lf),Nimx);
for kf=1:length(lf)
for Ni=1:Nimx
n=0.1;% Social norms
b=1/N;
...
tcross=zeros(1,Np);T_cross_avg=zeros(Np,length(lf));
for i=1:Np
    if i==1
        S(1,i)=K(i)-15;
    else
...
for i=1:Np
    T_cross(i,kf,Ni)=tcross(i);
end
% T_cross1(kf,Ni)=tcross(1);T_cross2(kf,Ni)=tcross(2);
% T_cross3(kf,Ni)=tcross(3);T_cross4(kf,Ni)=tcross(4);
...
for kf=1:length(lf)
    for i=1:Np
        if isempty(nonzeros(T_cross(i,kf,:)))
            T_cross_avg(i,kf)=Nt;
        else
            T_cross_avg(i,kf)=sum(T_cross(i,kf,:))/length(nonzeros(T_cross(i,kf,:)));% average
        end
    end
end
% for kf=1:length(lf)
%     if isempty(nonzeros(T_cross1(kf,:)))
%         T_cross1_avg(kf)=Nt;
%     else
%         T_cross1_avg(kf)=sum(T_cross1(kf,:))/length(nonzeros(T_cross1(kf,:)));% average
%     end
%     if isempty(nonzeros(T_cross2(kf,:)))
%         T_cross2_avg(kf)=Nt;
%     else
%         T_cross2_avg(kf)=sum(T_cross2(kf,:))/length(nonzeros(T_cross2(kf,:)));% average
%     end
%     if isempty(nonzeros(T_cross3(kf,:)))
%         T_cross3_avg(kf)=Nt;
%     else
%         T_cross3_avg(kf)=sum(T_cross3(kf,:))/length(nonzeros(T_cross3(kf,:)));% average
%     end
%     if isempty(nonzeros(T_cross4(kf,:)))
%         T_cross4_avg(kf)=Nt;
%     else
%         T_cross4_avg(kf)=sum(T_cross4(kf,:))/length(nonzeros(T_cross4(kf,:)));% average
%     end
% end
% figure
% hold on
% for i=1:Np
%     plot(lf,T_cross_avg(i,:)/52,'Color',[1/i i/Np (Np-i)/Np]);
% end
CODE_Figure(lf,T_cross_avg/52,'Time to Crosspatch');%,S2,S3,S4);
toc